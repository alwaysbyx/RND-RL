#!/usr/bin/env python3
"""Dynamic GPU scheduler for Gym TRPO experiments.

Monitors free VRAM on GPUs and dynamically schedules Gym experiments.
Gym tasks are lightweight (~2-3 GB VRAM each), so many can run per GPU.
...
Experiment matrix:
  5 Gym envs × 4 actor variants × 1 max_kl × 5 seeds = 100 runs

Usage:
  python scripts/train_trpo/run_trpo_gym_dynamic.py              # run
  python scripts/train_trpo/run_trpo_gym_dynamic.py --dry-run    # preview
"""
import argparse
import os
import subprocess
...
WANDB_PROJECT = "trpo_gym_experiments"  # change to your wandb project
...
def main():
    parser = argparse.ArgumentParser(description="Dynamic Gym TRPO scheduler")
