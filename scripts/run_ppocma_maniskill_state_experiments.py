#!/usr/bin/env python3
"""Script to run PPO-CMA experiments on multiple ManiSkill state environments."""
import os
import subprocess
import sys
import time
...
ENV_CONFIGS = [
    {"env_id": "PickCube-v1", "num_envs": 1024, "total_timesteps": 50_000_000, "extra": ""},
...
# Base config file
BASE_CONFIG = "configs/maniskill-state-ppocma.yaml"
...
def main():
    ...
    print("Starting PPO-CMA batch experiments for ManiSkill state environments")
    ...
    tasks = []
    for i, (env_config, seed) in enumerate(
        (e, s) for e in ENV_CONFIGS for s in SEEDS
    ):
        gpu_id = GPU_IDS[i % len(GPU_IDS)]
        tasks.append((env_config, seed, gpu_id))
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_TASKS) as executor:
        future_to_task = {}
        for env_config, seed, gpu_id in tasks:
